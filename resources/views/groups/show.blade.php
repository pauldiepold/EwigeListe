    ...
    <tabs>
        <tab name="Mitglieder" icon="fa-users" :selected="true">
                        ...
                        <table class="table myDataTable d-none table-responsive-sm">
                            <thead>
                                <tr class="border-bottom-thick" style="line-height:120%;">
                                    <th>
                                        <span class="text-transparent">Phot</span>
                                    </th>
                                    <th>Name</th>
                                    <th>Spiele</th>
                                    <th>Punkte</th>
                                        ...
                                        <td style="padding-right: 0px;">
                                            <a href="{{ $profile->path() }}">
                                                <img src="{{ $profile->player->user->avatar_path }}"
                                                     class="mx-auto h-10 w-10 sm:w-8 sm:h-8 m-0 rounded-full">
                                            </a>
                                        </td>
                                        <td>
                                            <a href="{{ $profile->path() }}">
            ...
            @elseif($group->players->count() == 1)
                <h5 class="mt-8">Bisher ist nur <a
                            href="{{ $group->profiles->first()->path() }}">{{ $group->players->first()->surname }}</a>
                    Mitglied dieser Liste.</h5>
            @else
                <h5 class="mt-8">Bisher wurde keine Runde auf dieser Liste gespielt.</h5>
            @endif
        ...
        @if($group->records->count() != 0)
            <tab name="Statistiken" icon="fa-chart-area">
                <template v-slot:default="props">
                    ...
                    <h4 class="mt-8 mb-4">Statistiken:</h4>
                    <div class="row justify-content-center">
                    ...
                    @if($group->rounds->count() >= 5)
                        <h4 class="mt-8">Anzahl der Spiele:</h4>
        ...
        @if($badges->count() != 0)
            <tab name="Abzeichen" icon="fa-award">
                <div class="sm:flex max-w-2xl mx-auto">
                    ...
                    @empty
                        <h5 class="mx-auto">Diese Runde hat noch keine Abzeichen.</h5>
                    @endforelse
                </div>
            </tab>
        @endif
            ...
            <tab name="Admin" icon="fa-cog">
                <div class="max-w-sm mx-auto">
                    @if(!$group->closed)
                        ...
                        <ul class="list-outside list-disc pl-8 text-left">
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('.myDataTable').forEach((table) => {
