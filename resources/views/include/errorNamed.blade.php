@if ($errors->$nameBag->any())
    @foreach ($errors->$nameBag->all() as $error)
        <div x-data="{ visible: true }" x-show="visible" class="my-3">
            <div class="flex items-center justify-between px-4 py-2 rounded border bg-red-100 text-red-800 border-red-300">
                <span>{{ $error }}</span>
