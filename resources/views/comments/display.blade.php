        ...
        <div class="card-footer py-2 px-2 text-muted" style="font-size: 0.8rem;">
            ...
            @if(isset($link))
                ...
                @elseif(!strcmp($comment->commentable_type, 'App\Profile'))
                    ...
                    in
                    <a href="/profiles/{{ $comment->commentable_id }}">{{ $surname }}{{ strcmp(substr($surname, -1), 's') ? 's' : '' }}</a>
                    Profil
                @endif
            @else
                @if(Auth()->user()->player->id == $comment->createdBy->id)
                    <form action="/comments/{{ $comment->id }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-danger btn btn-link py-0" style="font-size: 0.8rem;">Kommentar
                            löschen
                        </button>
                    </form>
                @endif
